...
@title: Multi-Class ChatGPT-Detector
...
for i in range(1, exam_answers + 1):    
    ...
    print("Corpus generated on multi-class (students + ChatGPT)")
    corpus = corpus_students[0:25] + corpus_chatGPT[0:25]
    vectorizer = TfidfVectorizer()
    vect = vectorizer.fit_transform(corpus)
    X_train = vect.toarray()
    y_train = y_students[0:25] + y_chatGPT[0:25]
    
    #Vectorize the test set using the previously generated corpus
    new_docs = corpus_students[25:36] + corpus_chatGPT[25:30]
    vect = vectorizer.transform(new_docs)
    X_test = vect.toarray()
    y_test = y_students[25:36] + y_chatGPT[25:30]
    
    #Random forest multi-class classifier
    print("Random forest classifier")
    clf = RandomForestClassifier(max_depth=5, n_estimators=200, max_features=50)
    ...
    print(metrics.classification_report(y_test, y_pred, target_names=['student','ChatGPT']))
    ...
    fig.savefig('CM_RF_trace' + str(i) + '.png', dpi=200)
    ...
    disp.plot(cmap=plt.cm.Blues,ax=ax)
    disp.ax_.set_title('Answer ' + str(i))
    fig.savefig('CM_norm_RF_trace' + str(i) + '.png', dpi=200)